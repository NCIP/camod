...
<%@ page import="gov.nih.nci.camod.webapp.form.BacteriaForm" %>	
...
<%
	...
	String actionName = "BacteriaAction.do?method=save";
	
	if ( aCarcinogenExposureID != null && aCarcinogenExposureID.length() > 0 && isDeleted == null) {
		actionName = "BacteriaAction.do?method=edit";
	}
	else {
...
<!-- -->
	...
	<tr>
		...
		<td class="formRequiredLabel"><label for="bName">Bacteria:</label></td>				
		<td class="formField">
			<br>
			(if Bacteria is not listed, then please<br>select "Other" from the list and specify it below)
			<br><br>
			<html:select styleClass="formFieldSized" size="1" styleId="bName" property="name"  onclick="chkOtherName();">												
				<html:options name="<%= Dropdowns.BACTERIADROP %>"/>					
			</html:select>			
		</td>
	</tr>
	...
	<tr>
		<td class="formRequiredNotice" width="5">&nbsp;</td>
		<td class="formLabel"><label for="otherName">Other Bacteria:</label></td>
		<td class="formField">
	...
	<tr>
		...
		<td class="formField">
			...
			<html:select styleClass="formFieldUnSized" size="1" styleId="dosageUnit" property="dosageUnit" >												
				<html:options name="<%= Dropdowns.BACTERIAUNITSDROP %>"/>					
			</html:select>	
 		</td>
	</tr>
