...
<%@ page import="gov.nih.nci.camod.webapp.form.SignalingMoleculeForm" %>	
...
<%
	...
	String actionName = "SignalingMoleculeAction.do?method=save";
	
	if ( aCarcinogenExposureID != null && aCarcinogenExposureID.length() > 0 && isDeleted == null) {
		actionName = "SignalingMoleculeAction.do?method=edit";
	}
	else {
...
<!-- -->
	...
	<tr>
		<td class="formTitle" height="20" colspan="3">Signaling Molecule 
			<camod:cshelp topic="signal_molecule_help" key="ignore" image="/camod/images/iconHelp.gif" text=""/></td>
	</tr>

	<tr>
		<td class="formRequiredNotice" width="5">*</td>
		<td class="formRequiredLabel"><label for="smName">Signaling Molecule:</label></td>				
		<td class="formField">
			<br>
			(if Signaling Molecule is not listed, then please<br>select "Other" from the list and specify it below)
			<br><br>
			<html:select styleClass="formFieldSized" size="1" styleId="smName" property="name"  onclick="chkOtherName();">												
				<html:options name="<%= Dropdowns.SIGNALINGMOLECULEDROP %>"/>					
			</html:select>			
		</td>
	</tr>
	...
	<tr>
		<td class="formRequiredNotice" width="5">&nbsp;</td>
		<td class="formLabel"><label for="otherName">Other Signaling Molecule:</label></td>
		<td class="formField">
	...
	<tr>
		...
		<td class="formField">
			...
			<html:select styleClass="formFieldUnSized" size="1" styleId="dosageUnit" property="dosageUnit" >												
				<html:options name="<%= Dropdowns.SIGNALINGMOLECULEUNITSDROP %>"/>					
			</html:select>	
 		</td>
	</tr>
