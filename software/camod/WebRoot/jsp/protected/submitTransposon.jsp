...
<%@ page import="gov.nih.nci.camod.webapp.form.TransposonForm" %>	
...
<%
	...
	String actionName = "TransposonAction.do?method=save";
	
	if ( aCarcinogenExposureID != null && aCarcinogenExposureID.length() > 0 && isDeleted == null) {
		actionName = "TransposonAction.do?method=edit";
	}
	else {
...
<!-- -->
	...
	<tr>
		<td class="formTitle" height="20" colspan="3">Transposon 
			<camod:cshelp topic="transposon_help" key="ignore" image="/camod/images/iconHelp.gif" text=""/></td>
	</tr>

	<tr>
		<td class="formRequiredNotice" width="5">*</td>
		<td class="formRequiredLabel"><label for="tName">Transposon:</label></td>				
		<td class="formField">
			<br>
			(if Transposon is not listed, then please<br>select "Other" from the list and specify it below)
			<br><br>
			<html:select styleClass="formFieldSized" size="1" styleId="tName" property="name"  onclick="chkOtherName();">												
				<html:options name="<%= Dropdowns.TRANSPOSONDROP %>"/>					
			</html:select>			
		</td>
	</tr>
	...
	<tr>
		<td class="formRequiredNotice" width="5">&nbsp;</td>
		<td class="formLabel"><label for="otherName">Other Transposon:</label></td>
		<td class="formField">
	...
	<tr>
		...
		<td class="formField">
			...
			<html:select styleClass="formFieldUnSized" size="1" styleId="dosageUnit" property="dosageUnit" >												
				<html:options name="<%= Dropdowns.TRANSPOSONUNITSDROP %>"/>					
			</html:select>	
 		</td>
	</tr>
