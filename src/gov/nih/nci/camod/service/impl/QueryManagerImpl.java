 ...
 * @author dgeorge
 * 
 * $Id: QueryManagerImpl.java,v 1.10 2005-10-05 13:04:47 georgeda Exp $
 * 
...
public class QueryManagerImpl extends BaseManager {
	...
	private String getModelIdsForTherapeuticApproach(String inTherapeuticApproach) throws PersistenceException {
		...
		Object[] theParams = new Object[1];
		theParams[0] = "%" + inTherapeuticApproach + "%";
		return getModelIds(theSQLString, theParams);
	...
	public List searchForAnimalModels(SearchData inSearchData) throws Exception {
		...
		System.out.println("Search therap: " + inSearchData.isSearchTherapeuticApproaches());
		System.out.println("Search therap: " + inSearchData.isSearchTherapeuticApproaches());
		...
		if (inSearchData.isSearchHistoMetastasis()) {
			theHQLQuery += " AND abs_cancer_model_id IN (" + getModelIdsForHistoMetastasis() + ")";
		}

		// Search for therapeutic approaches
		if (inSearchData.isSearchMicroArrayData()) {
			theHQLQuery += " AND abs_cancer_model_id IN (" + getModelIdsForMicroArrayData() + ")";
		}
...
/*
 * $Log: not supported by cvs2svn $ Revision 1.9 2005/10/04 20:19:43 georgeda
 * Updates from search changes Revision 1.6 2005/09/27 16:46:59 georgeda Added
 * environmental factor dropdown query Revision 1.5 2005/09/26 14:04:14 georgeda
 * Use HQL instead of SQL Revision 1.4 2005/09/16 19:30:04 guruswas Display
 * invivo data (from DTP) in the therapuetic approaches page
 * 
