...
/**
 * 
 * $Id: viewGeneticDescription_futureRelease.jsp,v 1.1 2005-12-05 20:28:46 pandyas Exp $
 * 
...
<bean:define id="tlcNames" name="<%=Constants.TARGETED_MOD_LIBRARY_NAMES_COLL%>"/>
	...
	<TABLE summary="" cellpadding="7" cellspacing="0" border="0" align="left" width="100%">
       ...
       <tr>
		...
		<c:if test="${not empty gene}">
		        ...
		        <br>
					<ul>
						<c:forEach var="item" items="${gene.pathwayCollection}">
							<li>
							<a target="_blank" href="http://cmap.nci.nih.gov/Pathways/BioCarta/<c:out value="${item.name}"/>">
	        ...
	        <!-- Libraries and Tissues (from EST data)  -->
	        <tr>
				...
				<!-- Remember to add libraryList action to struts config when development resumes  -->
	        	<font size="-2" color="#666699">This gene is found in these <a href="libraryList.do?aModelID=<c:out value="${mdl.id}"/>&tmId=<c:out value="${tm.id}"/>&CID=<c:out value="${gene.clusterId}"/>">libraries
		 		</a> from the following tissue types:</font> 
		 		<br><br>
		 		  
	            <c:forEach var="library" items="${gene.libraryCollection}">
	                 <c:out value="${library.tissue.name}"/>&nbsp;
	            </c:forEach> 
	            
	            <br><br>
	            <!-- Should be able to get LibraryCollection from ViewModelAction directly and remove duplicates (not finished) -->
	            <!-- Performance from caBio was so slow that we will move this to next release  -->	            
			  	<c:if test="${not empty targetedModLibraryNamesColl}"> 		 
			  		<c:forEach var="libraryNames" items="${targetedModLibraryNamesColl}">
			  			<c:out value="${libraryNames}"/>&nbsp;
			  		</c:forEach>
	  			</c:if>	            
	            
				<br><br>
				<a target="_blank" href="http://cgap.nci.nih.gov/Tissues/VirtualNorthern?TEXT=0&ORG=Mm&&CID=<c:out value="${gene.clusterId}"/>"> Monochromatic SAGE/cDNA Virtual Northern </a>			            
	            </a>
