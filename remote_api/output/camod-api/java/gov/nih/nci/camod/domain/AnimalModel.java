	   ...
	   public void setRepositoryInfo(gov.nih.nci.camod.domain.RepositoryInfo repositoryInfo){
		this.repositoryInfo = repositoryInfo;
	   	...
	   	public void setXenograftCollection(java.util.Collection xenograftCollection){
			...
			try{
			   if(carcinogenExposureCollection.size() == 0) {}
		           } catch(Exception e) {			     
			      ApplicationService applicationService = ApplicationServiceProvider.getApplicationService();
			      try {
				 	...
				 	gov.nih.nci.camod.domain.AnimalModel thisIdSet = new gov.nih.nci.camod.domain.AnimalModel();
			         	thisIdSet.setId(this.getId());
			         	java.util.Collection resultList = applicationService.search("gov.nih.nci.camod.domain.CarcinogenExposure", thisIdSet);				 
				 	carcinogenExposureCollection = resultList;  
				 	return resultList;
				 
			      
			      }catch(Exception ex) 
			      {
			      	System.out.println("AnimalModel:getCarcinogenExposureCollection throws exception ... ...");
			   		ex.printStackTrace(); 
			      }
			   }	
